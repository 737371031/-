...
- 按 `工具文件/WUZU-IMAGE.html` 的 API 调用文档修复 GPT Image 2 生图 skill：补齐自定义宽高、`output_width/output_height`、`aspect_ratio/resolution`、`response_format`、异步任务轮询和返回解析兼容。
...
- 固定 GPT Image 2 skill 的 API `model` 字段为 `gpt-image-2`，网页和 CLI 不再允许改模型名。
- 修复生图网页版生成接口仍向脚本传入旧 `model` 参数导致 `generate_image() got an unexpected keyword argument 'model'` 的 500 错误。
- 修复 GPT Image 2 选择 `quality=4k` 但未明确传 4K 分辨率的问题：CLI 和网页默认按质量档补 `aspect_ratio/resolution`，不再传额外模型配置字段，网页新增自动、手动最长边、手动宽高和原样传入四种分辨率控制，并展示请求尺寸与实际图片像素。
- 修正 GPT Image 2 分享包的 4K 传参逻辑：模型始终只传 `gpt-image-2`，4K 只通过 `resolution` 或 `width/height` 分辨率字段表达，并移除网页与 CLI 中的额外模型配置入口。
